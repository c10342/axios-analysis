...
xhr 请求处理函数的执行过程优】有点复杂，但是整体上可以归纳为：
...
通过`validateStatus`函数的分析，我们可以的出的结论是，如果响应状态码大于等于200并且小于300的，则请求是成功的，否则，请求是失败的
...
2、`withCredentials`属性是用来设置跨域的时候是否携带cookie，同域的时候不管设置或者不设置，效果都是一样的
